...
import { formatClock, formatDuration, roundSummaries } from '@/components/admin/admin-utils'
...
export default function PacingCard({ state }: { state: AdminState }) {
  ...
  const suggestion =
    status === 'playing' && current?.beganAt
      ? `Suggestion: end Round ${currentRound} around ${formatClock(new Date(current.beganAt.getTime() + perRoundMs))}`
  ...
  return (
    <section className="rounded-2xl border border-hairline bg-card p-5 shadow-card md:p-6">
      <div className="text-xs font-extrabold uppercase tracking-[0.10em] text-ink-soft">
        Suggested pace for 2–2.5 hours
      </div>
      <h2 className="mt-1 font-display text-2xl font-semibold text-ink">Round pacing</h2>

      {/* Round track dots */}
      <div className="mt-4 flex items-center gap-2.5" role="list" aria-label="Round track">
        {Array.from({ length: 6 }, (_, i) => {
          ...
          return (
            <motion.span
              ...
              title={`Round ${round}${done ? ' (done)' : currentDot ? ' (current)' : ''}`}
            >
              {currentDot && (
                <motion.span
        ...
        <label className="ml-auto flex items-center gap-2 text-sm font-bold text-ink-soft">
          ...
          <select
            ...
            aria-label="Planned number of rounds"
          >
            {[4, 5, 6].map((n) => (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-paper-deep text-left text-xs font-extrabold uppercase tracking-[0.08em] text-ink-soft">
                ...
                <th className="px-3 py-2">Deals signed</th>
              ...
              {summaries.map((s) => (
                <tr key={s.round} className="border-t border-hairline">
                  ...
                  <td className="px-3 py-2 text-ink-soft">
                    {s.beganAt && s.endedAt
                      ? formatDuration(s.endedAt.getTime() - s.beganAt.getTime())
