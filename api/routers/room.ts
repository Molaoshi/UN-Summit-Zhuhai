...
import { db, logActivity, requirePlayer } from "./helpers";
...
export const roomRouter = createRouter({
  ...
  create: publicQuery
    .input(z.object({ teacherName: z.string().trim().min(1).max(64) }))
    .mutation(async ({ input }) => {
      ...
      const [{ id: roomId }] = await d
        .insert(rooms)
        .values({ code, adminPin, status: "lobby", currentRound: 0 })
        .$returningId();
      ...
      // Seed round-0 bloc memberships with the starting blocs.
      ...
      return { token, roomCode: code, adminPin };
    }),
  ...
  join: publicQuery
    ...
    .mutation(async ({ input }) => {
      ...
      await logActivity(d, room, "lobby", `${input.name} joined the room.`);
